...
RSpec.describe Clover, "github" do
  describe "authentication" do
    ...
    it "vm has runner but no repository" do
      GithubRunner.create(vm_id: vm.id, repository_name: "test", label: "layerrail")
      get "/runtime/github"
    ...
    it "vm has runner and repository" do
      ...
      GithubRunner.create(vm_id: vm.id, repository_name: "test", label: "layerrail", repository_id: repository.id)
      get "/runtime/github"
  ...
  it "handles errors when attempting to setup blob storage" do
    ...
    GithubRunner.create(vm_id: vm.id, repository_name: "test2", label: "layerrail", repository_id: repository.id)
    blob_storage_client = instance_double(Aws::S3::Client)
    allow(Aws::S3::Client).to receive(:new).and_return(blob_storage_client)
    expect(blob_storage_client).to receive(:create_bucket)
    cloudflare_client = instance_double(CloudflareClient)
  ...
  describe "cache endpoints" do
    ...
    let(:runner) { GithubRunner.create(vm_id: create_vm.id, repository_name: "test", label: "layerrail", repository_id: repository.id, workflow_job: {head_branch: "dev"}) }
