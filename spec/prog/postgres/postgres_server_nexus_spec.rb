...
RSpec.describe Prog::Postgres::PostgresServerNexus do
  ...
  describe "#refresh_certificates" do
    ...
    it "pushes certificates to vm and hops to configure_prometheus during initial provisioning" do
      ...
      expect(sshable).to receive(:_cmd).with("sudo groupadd -f --system cert_readers")
      expect(sshable).to receive(:_cmd).with("sudo usermod -aG cert_readers postgres")
      expect(sshable).to receive(:_cmd).with("id -u prometheus >/dev/null 2>&1 && sudo usermod -aG cert_readers prometheus || true")
