...
RSpec.describe Kubernetes::NetworkReconciler, :no_db_transaction do
  ...
  it "carries pod-CIDR traffic between isolated Linux nodes over VXLAN" do
    ...
    begin
      ...
      run.call("ip netns exec #{left_ns} ip link set lo up; ip netns exec #{left_ns} ip link set #{left_node} name eth0; ip netns exec #{left_ns} ip addr add 192.0.2.1/24 dev eth0; ip netns exec #{left_ns} ip link set eth0 up")
      run.call("ip netns exec #{right_ns} ip link set lo up; ip netns exec #{right_ns} ip link set #{right_node} name eth0; ip netns exec #{right_ns} ip addr add 192.0.2.2/24 dev eth0; ip netns exec #{right_ns} ip link set eth0 up")
      ...
      run.call("ip netns exec #{left_ns} ping -c 1 -W 1 192.0.2.2")

      provider_location = Struct.new(:linode?, :azure?).new(true, false)
      left_nic = Struct.new(:private_ipv4, :private_ipv6).new(NetAddr::IPv4Net.parse("10.138.1.0/24"), NetAddr::IPv6Net.parse("fd40:1::/80"))
      right_nic = Struct.new(:private_ipv4, :private_ipv6).new(NetAddr::IPv4Net.parse("10.138.2.0/24"), NetAddr::IPv6Net.parse("fd40:2::/80"))
      left_vm = Struct.new(:location, :nics).new(provider_location, [left_nic])
      right_vm = Struct.new(:location, :nics).new(provider_location, [right_nic])
      ...
      addresses = {left.id => "192.0.2.1", right.id => "192.0.2.2"}
