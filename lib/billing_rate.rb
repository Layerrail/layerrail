...
class BillingRate
  ...
  def self.line_item_description(resource_type, resource_family, amount)
    ...
    when "MonitoringMetricsStorage"
      "#{amount.to_i} GiB Metrics Storage"
    when "MonitoringMetricsSamples"
      "Metrics Ingestion"
    when "MonitoringLogsStorage"
      "#{amount.to_i} GiB Log Storage"
    when "MonitoringLogsIngest"
      "Log Ingestion"
    when "MonitoringUptimeCheck"
      "#{amount.to_i} Uptime Check#{amount.to_i == 1 ? "" : "s"}"
    when "MonitoringAlert"
      "#{amount.to_i} Alert Rule#{amount.to_i == 1 ? "" : "s"}"
    when "IPAddress"
