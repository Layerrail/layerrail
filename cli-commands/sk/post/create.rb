...
UbiCli.on("sk").run_on("create") do
  ...
  banner "lr sk sk-name create public-key"

  help_example 'lr sk my-sk-name create "$(cat ~/.ssh/id_ed25519.pub)"'
  help_example 'lr sk my-sk-name create "$(cat ~/.ssh/authorized_keys)"'
