...
class Clover
  ...
  hash_branch(:webhook_prefix, "bachs") do |r|
    r.post true do
      ...
      result = BachsBillingVerificationCheckout.reconcile_event!(event) if result[:status] == "ignored"
    ...
    rescue BachsAPIError, BachsBillingVerificationCheckout::VerificationError => ex
      response.status = 503
